...
export default function ShopUnsoldPage() {
  ...
  // Optional customer details -- collapsed by default, entering anything here is never
  // required to mark a phone as sold.
  ...
  function openSoldModal(p: ShopPhoneView) {
    setSoldTarget(p);
    setSoldPrice("");
  ...
  async function submitSold(e: React.FormEvent) {
    ...
    setSoldSaving(true);
    try {
      // Customer details are only sent (and only saved) when the section was opened and at
      // least one field was filled in -- otherwise the phone sells with no customer record.
      const payload: { soldPrice: number; customer?: CustomerInfo } = { soldPrice: Number(soldPrice) };
      if (showCustomer && Object.values(customer).some((v) => v && String(v).trim())) {
        payload.customer = customer;
      }
      await api.patch(`/phones/${soldTarget._id}/sold`, payload);
      successToast("Marked as sold");
      setSoldTarget(null);
      setSoldPrice("");
      setCustomer({});
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
            {phones.map((p) => (
              <ShopPhoneCard key={p._id} phone={p}>
                <div className="flex gap-1.5 sm:gap-2">
                  <Button onClick={() => openSoldModal(p)} size="sm" className="flex-1 text-xs sm:text-sm px-2 sm:px-3">
                    Sold
                  </Button>
                  <Button
                    onClick={() => setIssueTarget(p)}
          ...
          <Modal title="Mark phone as sold" onClose={() => setSoldTarget(null)}>
            <form onSubmit={submitSold} className="space-y-4">
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-sm font-medium text-brand-700 dark:text-brand-400"
              >
              ...
              {showCustomer && <CustomerFields value={customer} onChange={setCustomer} />}
