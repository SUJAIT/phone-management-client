      ...
      <Card className="overflow-hidden flex flex-col hover:shadow-md transition-shadow">
        ...
        >
          {phone.images?.[0] ? (
            ...
            <div className="w-full h-full flex items-center justify-center text-slate-300 dark:text-slate-600">
              <Smartphone className="h-8 w-8 sm:h-14 sm:w-14" />
            </div>
          )}
          <div className="absolute top-1.5 left-1.5 sm:top-2 sm:left-2 flex gap-1 sm:gap-1.5">
            <Badge variant={statusVariant[phone.status]} className="capitalize text-[9px] sm:text-xs px-1.5 py-0 sm:px-2.5 sm:py-0.5">
              {phone.status}
            </Badge>
            {phone.hidden && <Badge variant="default" className="text-[9px] sm:text-xs px-1.5 py-0 sm:px-2.5 sm:py-0.5">Hidden</Badge>}
          </div>
        </button>

        <div className="p-2 sm:p-4 flex-1 flex flex-col gap-1 sm:gap-2">
          <button onClick={() => setDetailOpen(true)} className="text-left">
            <h3 className="text-sm sm:text-base font-semibold leading-snug line-clamp-2">{phone.name}</h3>
            {specs && <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 mt-0.5">{specs}</p>}
            <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 mt-0.5 truncate">IMEI: {phone.imei}</p>
            {ownerName && <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400">Added by: {ownerName}</p>}
          </button>
          ...
          <div className="mt-0.5 sm:mt-1">
            {phone.status === "sold" ? (
              <p className="text-sm sm:text-lg font-bold text-blue-600 dark:text-blue-400">
                {money(phone.soldPrice)}
                {isLossSale && (
                  <span className="ml-1 sm:ml-2 inline-block text-[8px] sm:text-[10px] font-semibold uppercase tracking-wide text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/40 rounded-full px-1 sm:px-1.5 py-0.5 align-middle">
                    Loss
                  </span>
                )}
              </p>
            ) : (
              <p className="text-sm sm:text-lg font-bold text-brand-700 dark:text-brand-400">
                {phone.sellExpectation}
                <span className="text-[10px] sm:text-xs font-normal text-slate-400 ml-1">expected</span>
              </p>
            )}
          </div>

          {phone.status === "issue" && phone.issueDescription && (
            <p className="text-[10px] sm:text-xs bg-red-50 text-red-700 dark:bg-red-950/40 dark:text-red-300 rounded-lg p-1.5 sm:p-2 line-clamp-2">
              Issue: {phone.issueDescription}
            </p>
          )}

          {phone.status === "loss" && (
            <p className="text-[10px] sm:text-xs bg-amber-50 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300 rounded-lg p-1.5 sm:p-2">
              Written off as a loss
            </p>
          )}

          {ownerActions && canManage && (
            <div className={`${children ? "" : "mt-auto"} pt-1 sm:pt-2 flex flex-col xs:flex-row gap-1.5 sm:gap-2`}>
              <Link href={`/phones/${phone._id}/edit`} className="flex-1" onClick={(e) => e.stopPropagation()}>
                <Button variant="secondary" size="sm" className="w-full text-xs sm:text-sm px-2">
